      ...
      <div className="min-h-screen flex items-center justify-center px-4">
        <Card className="w-full max-w-lg">
          <CardHeader>
            ...
            <CardDescription>This payment link is missing a valid recipient address.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="rounded-md bg-muted p-3 font-mono text-xs break-all">to={to || '(missing)'}</div>
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      <Card className="w-full max-w-lg">
        <CardHeader>
          ...
          <CardDescription>Sign in (or create a passkey) to send to:</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="rounded-md bg-muted p-3 font-mono text-xs break-all">{validTo}</div>

          <Button className="w-full" onClick={handleSignUp} disabled={isPending || isConnecting}>
            Create passkey (Touch ID)
          </Button>

          <Button variant="outline" className="w-full" onClick={handleSignIn} disabled={isPending || isConnecting}>
            Sign in with passkey
          </Button>

          <Button variant="secondary" className="w-full" onClick={handleFallback} disabled={isPending || isConnecting}>
